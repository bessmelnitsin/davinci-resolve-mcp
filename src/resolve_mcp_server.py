...
def get_whisper_status() -> Dict[str, Any]:
    ...
    config = transcription_operations.get_whisper_config() if hasattr(transcription_operations, 'get_whisper_config') else {}
    if not config:
    ...
    if create_timelines:
        ...
        for i, seg in enumerate(segments, 1):
            ...
            results.append({
                ...
                "score": score,
                "preview": preview,
                "status": "created" if "Timeline created" in result else result
            })
    else:
        for i, seg in enumerate(segments, 1):
            results.append({
...
def analyze_content(
    ...
    if not file_path:
        return "Error: Provide either file_path or clip_name"
...
# ------------------
# Viral Segment Detection (Opus Clip-like)
# ------------------
...
@mcp.resource("resolve://viral-analysis/{clip_name}")
def get_viral_analysis(clip_name: str) -> Dict[str, Any]:
    """
    Resource endpoint to get viral segment analysis for a clip.
    ...
    whisper_data = transcribe_with_whisper_node(file_path)
    if "error" in whisper_data:
        return {"error": f"Transcription failed: {whisper_data['error']}"}
    
    # Get viral segments
    segments = find_viral_segments_impl(whisper_data)
