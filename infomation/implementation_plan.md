# DaVinci Resolve MCP — Детальный план реализации

> **Проект:** MCP Server для автоматизации монтажа в DaVinci Resolve Studio  
> **Дата:** 31 декабря 2024  
> **Статус:** Анализ завершён, план готов к выполнению

---

## Обзор проекта

MCP (Model Context Protocol) сервер обеспечивает интеграцию AI-ассистентов (Cursor, Claude Desktop, Antigravity) с DaVinci Resolve Studio для автоматизации монтажа через естественный язык.

**Текущее состояние:**
- 202 функции объявлены реализованными
- Только 8% верифицированы на macOS
- 0% верифицированы на Windows
- 82% требуют тестирования

---

## Фаза 1: Критические исправления 🔴

> **Срок:** 1-2 дня  
> **Приоритет:** Блокирующие проблемы

---

### 1.1 Конфигурация и пути

#### [MODIFY] [whisper_node.py](file:///c:/GenModels/[Antigravity]/projects/test1/davinci-resolve-mcp/src/api/whisper_node.py)

**Проблема:** Захардкоженные абсолютные пути к Whisper

```python
# ТЕКУЩИЙ КОД (строки 10-11):
WHISPER_VENV_PYTHON = r"c:\GenModels\Whisper-WebUI\venv\Scripts\python.exe"
WHISPER_GEN_SCRIPT = r"C:\Users\Black\.gemini\whisper_to_json.py"
```

**Решение:**
1. Создать конфигурационный файл `config/whisper_config.json`
2. Добавить fallback на environment variables
3. Автоопределение системного Whisper

```python
# НОВЫЙ КОД:
import json
from pathlib import Path

def get_whisper_config():
    config_path = Path(__file__).parent.parent.parent / "config" / "whisper_config.json"
    defaults = {
        "whisper_python": os.environ.get("WHISPER_PYTHON", "python"),
        "whisper_script": os.environ.get("WHISPER_SCRIPT", ""),
        "model_size": "large-v3",
        "use_native_resolve_ai": True  # Fallback на встроенный AI DaVinci
    }
    if config_path.exists():
        with open(config_path) as f:
            return {**defaults, **json.load(f)}
    return defaults
```

#### [NEW] [whisper_config.json](file:///c:/GenModels/[Antigravity]/projects/test1/davinci-resolve-mcp/config/whisper_config.json)

```json
{
  "whisper_python": "python",
  "whisper_script": "",
  "model_size": "large-v3",
  "use_native_resolve_ai": true,
  "cache_transcriptions": true,
  "supported_languages": ["en", "ru", "auto"]
}
```

---

### 1.2 Незавершённые функции

#### [MODIFY] [ai_director.py](file:///c:/GenModels/[Antigravity]/projects/test1/davinci-resolve-mcp/src/api/ai_director.py)

**Проблема:** Функция `parse_ai_segments()` — пустая заглушка

```python
# ТЕКУЩИЙ КОД (строки 23-32):
def parse_ai_segments(ai_selection_text: str, whisper_data: dict) -> List[dict]:
    """Parse a list of segments suggested by AI."""
    pass  # <-- ПУСТАЯ ФУНКЦИЯ!
```

**Решение:** Реализовать парсинг нескольких форматов

```python
def parse_ai_segments(ai_selection_text: str, whisper_data: dict) -> List[dict]:
    """Parse AI-suggested segments from multiple formats.
    
    Supported formats:
    1. JSON: [{"start": 12.5, "end": 45.0, "title": "Hook"}]
    2. Text: "Reel 1: 12.5 - 45.0 (Hook about coding)"
    3. Segment indices: "Segments: 0, 3, 5-8, 12"
    """
    segments = []
    
    # Try JSON first
    try:
        parsed = json.loads(ai_selection_text)
        if isinstance(parsed, list):
            return parsed
    except json.JSONDecodeError:
        pass
    
    # Parse text format: "Reel N: start - end (description)"
    reel_pattern = r"(?:Reel\s*\d*:?)?\s*(\d+\.?\d*)\s*[-–]\s*(\d+\.?\d*)\s*(?:\(([^)]*)\))?"
    matches = re.findall(reel_pattern, ai_selection_text)
    for match in matches:
        segments.append({
            "start": float(match[0]),
            "end": float(match[1]),
            "title": match[2] if match[2] else f"Segment {len(segments)+1}"
        })
    
    # Parse segment indices: "0, 3, 5-8"
    if not segments and whisper_data:
        whisper_segments = whisper_data.get("segments", [])
        index_pattern = r"(\d+)(?:\s*[-–]\s*(\d+))?"
        for match in re.findall(index_pattern, ai_selection_text):
            start_idx = int(match[0])
            end_idx = int(match[1]) if match[1] else start_idx
            for idx in range(start_idx, min(end_idx + 1, len(whisper_segments))):
                seg = whisper_segments[idx]
                segments.append({
                    "start": seg.get("start", 0),
                    "end": seg.get("end", 0),
                    "title": seg.get("text", "")[:50]
                })
    
    return segments
```

---

### 1.3 Исправление Render Pipeline

#### [MODIFY] [delivery_operations.py](file:///c:/GenModels/[Antigravity]/projects/test1/davinci-resolve-mcp/src/api/delivery_operations.py)

**Проблема:** `AddRenderJob` возвращает `'NoneType' object is not callable`

**Решение:** Добавить проверку API версии и fallback

```python
def add_to_render_queue(resolve, preset_name: str, ...):
    # ... существующий код ...
    
    # ДОБАВИТЬ: Проверка доступности метода
    project = resolve.GetProjectManager().GetCurrentProject()
    
    # Метод 1: Стандартный API
    if hasattr(project, 'AddRenderJob') and callable(getattr(project, 'AddRenderJob')):
        job_id = project.AddRenderJob()
        if job_id:
            return {"success": True, "job_id": job_id}
    
    # Метод 2: Через RenderSettings (Resolve 18+)
    render_settings = project.GetRenderFormats()
    if render_settings:
        # Альтернативный workflow через LoadRenderPreset
        if project.LoadRenderPreset(preset_name):
            job_id = project.AddRenderJob()
            return {"success": True, "job_id": job_id, "method": "preset_load"}
    
    # Метод 3: Manual settings
    return {"error": "AddRenderJob unavailable", 
            "suggestion": "Try switching to Deliver page first: resolve.OpenPage('deliver')"}
```

---

## Фаза 2: Рефакторинг и стабилизация 🟡

> **Срок:** 3-5 дней  
> **Приоритет:** Улучшение maintainability

---

### 2.1 Декомпозиция главного файла

#### [MODIFY] [resolve_mcp_server.py](file:///c:/GenModels/[Antigravity]/projects/test1/davinci-resolve-mcp/src/resolve_mcp_server.py)

**Проблема:** 4900 строк в одном файле с дублированием кода из `src/api/`

**План рефакторинга:**

| Секция | Строки | Вынести в модуль |
|--------|--------|------------------|
| Project Management | 356-658 | `api/project_operations.py` ✅ уже есть |
| Timeline Operations | 664-852 | `api/timeline_operations.py` ✅ уже есть |
| Media Pool | 858-1094 | `api/media_operations.py` ✅ уже есть |
| Color Page | 1100-1161 | `api/color_operations.py` ✅ уже есть |
| Delivery | 1167-1427 | `api/delivery_operations.py` ✅ уже есть |
| Transcription | 1430-1603 | **НОВЫЙ: `api/transcription_operations.py`** |
| Gallery/Stills | ~2000-2500 | **НОВЫЙ: `api/gallery_operations.py`** |
| Keyframes | ~2500-3000 | **НОВЫЙ: `api/keyframe_operations.py`** |
| Export/Import | ~3000-3500 | **НОВЫЙ: `api/export_operations.py`** |

**Новая структура resolve_mcp_server.py (~500 строк):**

```python
#!/usr/bin/env python3
"""DaVinci Resolve MCP Server - Main Entry"""

from mcp.server.fastmcp import FastMCP
from src.api import (
    project_operations,
    timeline_operations,
    media_operations,
    color_operations,
    delivery_operations,
    transcription_operations,  # NEW
    gallery_operations,        # NEW
    audio_operations,
    smart_editing
)

mcp = FastMCP("davinci-resolve")

# Регистрация tools через декораторы
@mcp.tool()
def get_resolve_version(): 
    return project_operations.get_version(resolve)

@mcp.tool()
def list_timelines():
    return timeline_operations.list_timelines(resolve)

# ... остальные tools по аналогии
```

---

### 2.2 Удаление дубликатов

#### [DELETE] [scripts/resolve_mcp_server.py](file:///c:/GenModels/[Antigravity]/projects/test1/davinci-resolve-mcp/scripts/resolve_mcp_server.py)

**Причина:** Дублирует функциональность `src/resolve_mcp_server.py`

---

### 2.3 Централизованная обработка ошибок

#### [NEW] [error_handling.py](file:///c:/GenModels/[Antigravity]/projects/test1/davinci-resolve-mcp/src/utils/error_handling.py)

```python
"""Централизованная обработка ошибок DaVinci Resolve API"""

from functools import wraps
from typing import Callable, Any
import logging

logger = logging.getLogger("davinci-resolve-mcp.errors")

class ResolveAPIError(Exception):
    """Базовый класс для ошибок API"""
    pass

class ResolveNotConnectedError(ResolveAPIError):
    """DaVinci Resolve не запущен или не подключен"""
    pass

class ProjectNotOpenError(ResolveAPIError):
    """Проект не открыт"""
    pass

class TimelineNotFoundError(ResolveAPIError):
    """Timeline не найден"""
    pass

class ClipNotFoundError(ResolveAPIError):
    """Клип не найден в Media Pool"""
    pass

def require_resolve(func: Callable) -> Callable:
    """Декоратор: проверяет подключение к Resolve"""
    @wraps(func)
    def wrapper(resolve, *args, **kwargs):
        if resolve is None:
            raise ResolveNotConnectedError(
                "DaVinci Resolve не подключен. Убедитесь что приложение запущено."
            )
        return func(resolve, *args, **kwargs)
    return wrapper

def require_project(func: Callable) -> Callable:
    """Декоратор: проверяет открытый проект"""
    @wraps(func)
    def wrapper(resolve, *args, **kwargs):
        project = resolve.GetProjectManager().GetCurrentProject()
        if project is None:
            raise ProjectNotOpenError(
                "Проект не открыт. Используйте open_project() или create_project()."
            )
        return func(resolve, *args, **kwargs)
    return wrapper

def safe_api_call(default_return: Any = None):
    """Декоратор: безопасный вызов API с логированием"""
    def decorator(func: Callable) -> Callable:
        @wraps(func)
        def wrapper(*args, **kwargs):
            try:
                return func(*args, **kwargs)
            except ResolveAPIError:
                raise
            except Exception as e:
                logger.error(f"{func.__name__} failed: {e}")
                return default_return
        return wrapper
    return decorator
```

---

## Фаза 3: Новые функции 🟢

> **Срок:** 1-2 недели  
> **Приоритет:** Расширение возможностей

---

### 3.1 Fairlight Page Operations (Аудио)

#### [NEW] [fairlight_operations.py](file:///c:/GenModels/[Antigravity]/projects/test1/davinci-resolve-mcp/src/api/fairlight_operations.py)

```python
"""DaVinci Resolve Fairlight Page Operations"""

import logging
from typing import Dict, Any, List, Optional

logger = logging.getLogger("davinci-resolve-mcp.fairlight")

def get_audio_tracks(resolve, timeline_name: str = None) -> List[Dict]:
    """Получить список аудиодорожек таймлайна."""
    pass

def set_track_volume(resolve, track_index: int, volume_db: float) -> str:
    """Установить громкость дорожки в dB (-96 to +12)."""
    pass

def add_audio_effect(resolve, track_index: int, effect_name: str) -> str:
    """Добавить аудиоэффект на дорожку."""
    # Доступные эффекты: "Compressor", "EQ", "Limiter", "DeEsser", etc.
    pass

def normalize_audio(resolve, clip_name: str, target_lufs: float = -14.0) -> str:
    """Нормализация громкости клипа по LUFS стандарту."""
    pass

def remove_silence(resolve, clip_name: str, threshold_db: float = -40) -> List[Dict]:
    """Определить и вернуть тихие участки для удаления."""
    pass

def voice_isolation(resolve, clip_name: str) -> str:
    """Использовать DaVinci Neural Engine для изоляции голоса."""
    # Требует DaVinci Resolve Studio
    pass
```

---

### 3.2 Fusion Page Operations

#### [NEW] [fusion_operations.py](file:///c:/GenModels/[Antigravity]/projects/test1/davinci-resolve-mcp/src/api/fusion_operations.py)

```python
"""DaVinci Resolve Fusion Page Operations"""

def get_fusion_comp(resolve, clip_name: str = None):
    """Получить Fusion композицию текущего клипа."""
    pass

def add_text_node(resolve, text: str, font: str = "Arial", size: int = 100) -> str:
    """Добавить текстовый узел в Fusion."""
    pass

def add_blur_effect(resolve, blur_size: float = 5.0) -> str:
    """Добавить эффект размытия."""
    pass

def create_lower_third(resolve, name: str, title: str, subtitle: str = "") -> str:
    """Создать нижнюю плашку с именем/титулом."""
    pass

def apply_template(resolve, template_name: str, **params) -> str:
    """Применить Fusion template с параметрами."""
    pass
```

---

### 3.3 Smart Automation Tools

#### [MODIFY] [smart_editing.py](file:///c:/GenModels/[Antigravity]/projects/test1/davinci-resolve-mcp/src/api/smart_editing.py)

**Добавить новые функции:**

```python
def auto_color_match(resolve, reference_clip: str, target_clips: List[str]) -> str:
    """Автоматическое согласование цвета по референсу."""
    pass

def smart_reframe(resolve, clip_name: str, aspect_ratio: str = "9:16") -> str:
    """Умное кадрирование с отслеживанием лица/объекта."""
    # Использует DaVinci Neural Engine
    pass

def auto_subtitle(resolve, clip_name: str, style: str = "default") -> str:
    """Автоматическое создание субтитров из транскрипции."""
    pass

def batch_export_reels(resolve, timeline_name: str, 
                       segment_markers: bool = True) -> List[str]:
    """Экспорт таймлайна по маркерам в отдельные файлы."""
    pass

def detect_scenes(resolve, clip_name: str, sensitivity: float = 0.5) -> List[Dict]:
    """Определение смены сцен в клипе."""
    pass

def auto_music_sync(resolve, video_clip: str, music_clip: str) -> str:
    """Синхронизация нарезки под ритм музыки."""
    # Анализ BPM и создание маркеров
    pass
```

---

## Фаза 4: Тестирование и документация 📋

> **Срок:** Постоянно  
> **Приоритет:** Качество кода

---

### 4.1 Unit Tests

#### [NEW] [tests/unit/](file:///c:/GenModels/[Antigravity]/projects/test1/davinci-resolve-mcp/tests/unit/)

**Структура тестов:**

```
tests/
├── unit/
│   ├── test_ai_director.py      # Тесты parse_ai_segments
│   ├── test_whisper_node.py     # Тесты с mock Whisper
│   ├── test_jump_cut.py         # Тесты генерации монтажных точек
│   └── test_error_handling.py   # Тесты декораторов
├── integration/
│   ├── test_project_operations.py
│   ├── test_timeline_operations.py
│   └── test_media_operations.py
├── conftest.py                  # Pytest fixtures
└── mocks/
    └── resolve_mock.py          # Mock DaVinci Resolve API
```

#### [NEW] [conftest.py](file:///c:/GenModels/[Antigravity]/projects/test1/davinci-resolve-mcp/tests/conftest.py)

```python
import pytest
from unittest.mock import MagicMock

@pytest.fixture
def mock_resolve():
    """Mock DaVinci Resolve instance."""
    resolve = MagicMock()
    
    # Project Manager
    pm = MagicMock()
    pm.GetCurrentProject.return_value = MagicMock()
    pm.GetProjectListInCurrentFolder.return_value = ["Project1", "Project2"]
    resolve.GetProjectManager.return_value = pm
    
    # Media Pool
    mp = MagicMock()
    mp.GetRootFolder.return_value = MagicMock()
    resolve.GetMediaPool.return_value = mp
    
    return resolve

@pytest.fixture
def sample_whisper_data():
    """Sample Whisper transcription output."""
    return {
        "segments": [
            {"start": 0.0, "end": 5.0, "text": "Hello world"},
            {"start": 5.5, "end": 10.0, "text": "This is a test"},
            {"start": 15.0, "end": 20.0, "text": "After silence"},
        ],
        "language": "en"
    }
```

---

### 4.2 Документация API

#### [NEW] [docs/API_REFERENCE.md](file:///c:/GenModels/[Antigravity]/projects/test1/davinci-resolve-mcp/docs/API_REFERENCE.md)

Автогенерация документации на основе docstrings с помощью `pdoc` или `mkdocs`.

---

## Чеклист задач

### Фаза 1: Критические исправления
- [ ] Вынести пути Whisper в конфиг
- [ ] Реализовать `parse_ai_segments()`
- [ ] Исправить `AddRenderJob`
- [ ] Добавить fallback на Native Resolve AI

### Фаза 2: Рефакторинг
- [ ] Разбить `resolve_mcp_server.py` на модули
- [ ] Удалить `scripts/resolve_mcp_server.py`
- [ ] Создать централизованную обработку ошибок
- [ ] Добавить декораторы `@require_resolve`, `@require_project`

### Фаза 3: Новые функции
- [ ] Fairlight operations (audio)
- [ ] Fusion operations (effects)
- [ ] Smart Reframe
- [ ] Auto Subtitles
- [ ] Scene Detection
- [ ] Music Sync

### Фаза 4: Тестирование
- [ ] Создать mock Resolve API
- [ ] Unit тесты для ai_director
- [ ] Unit тесты для jump_cut
- [ ] Integration тесты
- [ ] CI/CD pipeline (GitHub Actions)

---

## Ресурсы

- [DaVinci Resolve Scripting API](https://documents.blackmagicdesign.com/UserManuals/DaVinci-Resolve-Scripting-Guide.pdf)
- [MCP Protocol Specification](https://modelcontextprotocol.io/)
- [Whisper OpenAI](https://github.com/openai/whisper)
